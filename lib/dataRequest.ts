import { getDataFromRequest, getDataRequestProgress, submitDataRequest } from "./actions/dataRequest";
...
export async function dataRequest(geometry: Geometry){
    ...
    if(dataReq.error != null){
        return {dataTemperature: null, dataPrecipitation: null, error: dataReq.error}
    }

    return {
        dataTemperature: await processDataTemperature(dataReq.dataTemperature),
        dataPrecipitation: await processDataPrecipitation(dataReq.dataPrecipitation),
        error: null
    }
}
...
export async function submitAllDataRequest(geometry: Geometry) {
    ...
    const processBatch = async (batch: number[]) => {
      const results = await Promise.allSettled(batch.map(async (datatype) => {
        try {
